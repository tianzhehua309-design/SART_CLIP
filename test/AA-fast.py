...
def _try_get_all_labels(dataset: Dataset) -> Optional[List[int]]:
    if hasattr(dataset, "targets"):
        try:
            return [int(y) for y in list(dataset.targets)]
        except Exception:
            pass

    if hasattr(dataset, "labels"):
        try:
            return [int(y) for y in list(dataset.labels)]
        except Exception:
            pass

    if hasattr(dataset, "_labels"):
        try:
            return [int(y) for y in list(dataset._labels)]
        except Exception:
            pass
...
def make_balanced_subset(
    dataset: Dataset, max_examples: Optional[int], seed: int = 42
) -> Dataset:
    if max_examples is None or max_examples >= len(dataset):
        return dataset

    rng = random.Random(seed)
    labels = _try_get_all_labels(dataset)
...
def build_loader(dataset, dataset_cfg: Dict, batch_size: int) -> DataLoader:
    if MAX_EXAMPLES_PER_DATASET is not None:
        ...
        print(
            f"[Subset] balanced max_examples={MAX_EXAMPLES_PER_DATASET}, actual={len(dataset)}"
        )
...
def prepare_classifier(
    ...
    if USE_PROMPT_ENSEMBLE_HEAD:
        text_banks, bank_names = build_prompt_banks_for_dataset(
            clip_model, dataset_cfg, raw_classes, resolved_classes, sem_json, device
        )

        print(f"[PromptHead] requested={PROMPT_BANKS_TO_USE}, actual={bank_names}")
    else:
        feats = []
...
def make_autoattack(
    ...
    if attack_preset == "fast":
        if num_classes <= 2:
            adversary.attacks_to_run = ["apgd-ce"]
        else:
            adversary.attacks_to_run = ["apgd-ce", "apgd-dlr"]

    elif attack_preset == "standard":
        if num_classes <= 2:
            adversary.attacks_to_run = ["apgd-ce"]
        else:
            pass
        ...
        raise ValueError(f"Unknown ATTACK_PRESET: {attack_preset}")

    print(
        f"[AutoAttack] num_classes={num_classes}, attacks_to_run={adversary.attacks_to_run}"
    )
    return adversary
...
def evaluate_autoattack(
    ...
    num_classes = int(classifier.text_banks.shape[1])

    adversary = make_autoattack(
        model=classifier,
    ...
    pbar = tqdm(
        loader, desc=f"[AA-{ATTACK_PRESET}] {dataset_name} | {method_name}", leave=False
    )
    ...
    for images, labels in pbar:
        ...
        x_adv = adversary.run_standard_evaluation(
            images,
            labels,
            bs=min(ATTACK_BATCH_SIZE, images.size(0)),
            return_labels=False,
        )
...
def main() -> None:
    ...
    for dataset_cfg in DATASETS:
        ...
        loader = build_loader(dataset, dataset_cfg, EVAL_BATCH_SIZE)

        for method_cfg in METHODS:
            print("-" * 120)
            print(f"Evaluating method: {method_cfg['name']}")
            classifier = prepare_classifier(method_cfg, dataset_cfg, dataset, DEVICE)

            clean_acc = evaluate_clean(
                classifier, loader, dataset_cfg["name"], method_cfg["name"]
            )
            robust_acc = evaluate_autoattack(
                classifier, loader, dataset_cfg["name"], method_cfg["name"]
            )

            print(
                f"[{dataset_cfg['name']}] {method_cfg['name']} | clean={clean_acc:.4f} | {ATTACK_PRESET}_aa={robust_acc:.4f}"
            )
            rows.append(
                {
